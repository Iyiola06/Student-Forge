...
const STORAGE_KEY = 'Sulvas_Studify_saved_materials';
    ...
    return (
        <div className="space-y-12 max-w-5xl mx-auto">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        {savedMaterials.map((m) => (
                            <div
                                ...
                                className="group relative bg-white dark:bg-[#1b1b27] p-5 rounded-2xl border border-slate-100 dark:border-slate-800/50 shadow-sm hover:shadow-xl hover:shadow-orange-500/10 hover:border-[#1a5c2a]/50 transition-all cursor-pointer"
                            >
                                <button
                                ...
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 transition-transform group-hover:scale-110 ${m.type === 'file' ? 'bg-orange-50 dark:bg-[#1a5c2a]/10 text-[#144823]' : 'bg-blue-50 dark:bg-blue-500/10 text-blue-500'}`}>
            ...
            <div className="bg-white dark:bg-[#1b1b27] p-6 md:p-12 rounded-[2.5rem] shadow-2xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800/50 relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-64 h-64 bg-[#1a5c2a]/5 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl group-hover:bg-[#1a5c2a]/10 transition-colors duration-1000"></div>
                ...
                <div className="space-y-8">
                    <div
                        ...
                        className={`relative group border-2 border-dashed rounded-[2rem] p-10 md:p-16 text-center transition-all cursor-pointer ${isDragging
                            ? 'border-[#1a5c2a] bg-[#1a5c2a]/10 scale-[1.02] ring-8 ring-orange-500/5'
                            : fileName
                    ...
                    >
                        ...
                        <div className="flex flex-col items-center">
                            <div className={`w-16 h-16 md:w-20 md:h-20 rounded-2xl flex items-center justify-center mb-6 transition-all duration-500 shadow-xl ${isDragging ? 'bg-[#1a5c2a] text-white scale-110 rotate-12' :
                                fileName ? 'bg-emerald-500 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 group-hover:bg-[#1a5c2a] group-hover:text-white group-hover:rotate-6'
                                }`}>
                    ...
                    <button
                        ...
                        className="w-full h-16 md:h-20 bg-[#1a5c2a] text-white text-lg md:text-xl font-black rounded-2xl md:rounded-[1.5rem] hover:bg-[#144823] disabled:bg-slate-100 dark:disabled:bg-slate-800 disabled:text-slate-400 transition-all shadow-2xl shadow-orange-500/20 flex items-center justify-center group active:scale-[0.98]"
                    >
                        {isProcessing ? (
