...
const ChapterChat: React.FC<{ chapter: StudyChapter; sourceText: string }> = ({ chapter, sourceText }) => {
    ...
    if (!isOpen) return (
        <button onClick={() => setIsOpen(true)} className="mt-8 flex items-center gap-3 text-xs font-black uppercase text-[#144823] bg-[#1a5c2a]/5 px-6 py-3 rounded-2xl border border-[#1a5c2a]/10 hover:bg-[#1a5c2a]/10 transition-all group">
        ...
        <div className="mt-10 bg-slate-900 border border-slate-800 rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-6 duration-500">
            <div className="p-6 bg-slate-800/50 flex justify-between items-center border-b border-slate-700">
                <div className="flex items-center gap-3 font-black text-xs text-white uppercase tracking-widest">
                    <span className="material-symbols-outlined text-[#1a5c2a]">school</span>
                    Context Session
                </div>
            ...
            <div className="p-8 h-80 overflow-y-auto space-y-6 flex flex-col no-scrollbar">
                {history.map((msg, i) => (
                    <div key={i} className={`flex items-start gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                        <div className={`max-w-[85%] px-5 py-4 rounded-2xl text-sm font-bold leading-relaxed ${msg.role === 'user' ? 'bg-[#1a5c2a] text-white rounded-br-none shadow-lg shadow-orange-500/20' : 'bg-slate-800 text-slate-200 border border-slate-700 rounded-bl-none'}`}>
                            {msg.parts[0].text}
                        </div>
                ...
                {isLoading && (
                    <div className="flex gap-2 p-4 bg-slate-800/30 rounded-2xl w-fit animate-pulse transition-all">
                        <div className="w-2 h-2 bg-[#1a5c2a] rounded-full animate-bounce"></div>
                        <div className="w-2 h-2 bg-[#1a5c2a] rounded-full animate-bounce delay-75"></div>
                        <div className="w-2 h-2 bg-[#1a5c2a] rounded-full animate-bounce delay-150"></div>
                    </div>
                )}
            </div>
            <form onSubmit={handleSend} className="p-6 border-t border-slate-800 flex gap-4 bg-slate-800/20">
                <input
                    ...
                    className="flex-grow bg-slate-800 text-white text-sm font-bold px-6 py-4 rounded-2xl border border-slate-700 outline-none focus:border-[#1a5c2a] transition-colors"
                />
                <button type="submit" disabled={!question.trim()} className="bg-[#1a5c2a] text-white w-14 h-14 rounded-2xl flex items-center justify-center shadow-xl shadow-orange-500/20 transition-all active:scale-90 hover:bg-[#144823] disabled:grayscale">
    ...
    return (
        <div className="max-w-5xl mx-auto px-4 pb-32 animate-in fade-in duration-1000">
            <div className="flex justify-between items-center mb-12">
                <button onClick={onRestart} className="px-6 py-3 bg-white dark:bg-[#1b1b27] rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 text-xs font-black text-slate-500 dark:text-slate-400 hover:text-[#1a5c2a] transition-all flex items-center gap-3">
                    <span className="material-symbols-outlined text-sm">arrow_back</span>
                    RECONFIGURE SOURCE
                </button>
                <div className="bg-[#1a5c2a] text-white px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-3 shadow-xl shadow-orange-500/20">
            ...
            <div className="space-y-20">
                ...
                <div className="bg-white dark:bg-[#1b1b27] p-10 md:p-16 rounded-[3rem] shadow-2xl border border-slate-100 dark:border-slate-800 relative overflow-hidden group">
                    <div className="absolute top-0 right-0 w-80 h-80 bg-[#1a5c2a]/5 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl opacity-50 group-hover:bg-[#1a5c2a]/10 transition-colors duration-1000"></div>

                    <div className="flex items-center gap-4 text-[#144823] font-black uppercase text-xs tracking-[0.3em] mb-8">
                    ...
                    <p className="text-slate-600 dark:text-slate-400 text-xl font-bold leading-relaxed italic border-l-[12px] border-[#1a5c2a]/20 pl-10 mb-12 relative">
                        <span className="text-[#1a5c2a] text-6xl absolute -left-6 -top-6 opacity-10 font-serif">"</span>
                    ...
                    <div className="flex flex-wrap gap-4">
                        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center gap-4">
                            <span className="material-symbols-outlined text-[#1a5c2a]">schedule</span>
                            <span className="text-slate-900 dark:text-white font-black text-sm">{breakdown.estimatedStudyTimeMinutes} MINS INTENSITY</span>
                        </div>
                        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center gap-4">
                            <span className="material-symbols-outlined text-[#1a5c2a]">list_alt</span>
                    ...
                    {breakdown.chapters.map((chapter, idx) => (
                        <div key={idx} className="group/chapter animate-in fade-in slide-in-from-bottom-12 duration-1000" style={{ animationDelay: `${idx * 150}ms` }}>
                            <div className="flex items-center gap-6 mb-10 pl-2">
                                <div className="w-16 h-16 bg-[#1a5c2a] text-white rounded-3xl flex items-center justify-center text-2xl font-black shadow-2xl shadow-orange-500/30 group-hover/chapter:scale-110 transition-transform cursor-default">
                                    {idx + 1}
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="md:col-span-2 bg-gradient-to-br from-[#1b1b27] to-[#101022] p-10 md:p-14 rounded-[3rem] text-white shadow-2xl relative overflow-hidden group/card border border-white/5">
                                    <div className="absolute top-0 right-0 w-64 h-64 bg-[#1a5c2a]/20 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl opacity-20 pointer-events-none"></div>
                                    <div className="flex items-center justify-between mb-8 relative z-10">
                                        <div className="px-4 py-1.5 bg-[#1a5c2a] text-white rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                                ...
                                {chapter.keyPoints.map((point, pIdx) => (
                                    <div key={pIdx} className="bg-white dark:bg-[#1b1b27] p-8 md:p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm hover:border-[#1a5c2a]/30 transition-all group/point">
                                        <div className="flex items-start justify-between mb-6">
                                            <div className="w-10 h-10 bg-orange-50 dark:bg-[#1a5c2a]/10 text-[#144823] rounded-xl flex items-center justify-center group-hover/point:scale-110 transition-transform">
                ...
                <div className="pt-20 text-center">
                    <button
                        onClick={onStartQuiz}
                        className="px-16 h-20 bg-[#1a5c2a] text-white font-black text-xl rounded-full hover:bg-[#144823] transition-all shadow-2xl shadow-orange-500/30 active:scale-95 group flex items-center gap-6 mx-auto"
                    >
