        ...
        <div className="px-6 pt-6 pb-2 md:px-8 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 bg-slate-100 dark:bg-[#252535] px-3 py-1.5 rounded-full border border-slate-200 dark:border-[#2d2d3f]">
              <span className="text-sm font-bold text-[#2525f4]">Level {profile?.level || 1}</span>
          ...
          <div className="max-w-4xl mx-auto space-y-8">
            {isLoading ? (
              <div className="py-20 flex justify-center items-center gap-3 text-slate-500 dark:text-[#9c9cba]">
                <div className="size-6 border-4 border-[#2525f4] border-t-transparent rounded-full animate-spin"></div>
                    ...
                    {top3[1] ? (
                      <div className="flex flex-col items-center">
                        ...
                        <div className="text-center w-24">
                          ...
                          <p className="text-sm text-[#2525f4] font-bold">{top3[1].xp?.toLocaleString()} XP</p>
                        </div>
                    ...
                    {top3[0] && (
                      <div className="flex flex-col items-center z-10 w-24 md:w-32">
                        ...
                        <div className="text-center w-full px-1">
                          ...
                          <p className="text-sm text-[#2525f4] font-bold">{top3[0].xp?.toLocaleString()} XP</p>
                        </div>
                    ...
                    {top3[2] ? (
                      <div className="flex flex-col items-center">
                        ...
                        <div className="text-center w-24">
                          ...
                          <p className="text-sm text-[#2525f4] font-bold">{top3[2].xp?.toLocaleString()} XP</p>
                        </div>
                ...
                <div className="bg-white dark:bg-[#1b1b27] rounded-xl border border-slate-200 dark:border-[#2d2d3f] overflow-hidden">
                  <div className="p-4 border-b border-slate-200 dark:border-[#2d2d3f] flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <button className="px-3 py-1 text-xs font-bold bg-[#2525f4] text-white rounded-lg">All Time</button>
                    </div>
                  </div>
                  <div className="divide-y divide-slate-100 dark:divide-[#2d2d3f]">
                    {others.length > 0 ? others.map((user, index) => {
                      ...
                      return (
                        <div key={user.id} className={`flex items-center gap-4 p-4 transition-colors ${isMe ? 'bg-[#2525f4]/5' : 'hover:bg-slate-50 dark:hover:bg-[#252535]'}`}>
                          <span className={`w-8 text-center font-bold ${isMe ? 'text-[#2525f4]' : 'text-slate-400'}`}>{rank}</span>
                          ...
                          <span className={`font-bold ${isMe ? 'text-[#2525f4]' : 'text-slate-600 dark:text-slate-300'}`}>
